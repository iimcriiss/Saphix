...
<div class="flex items-center gap-3 mb-6">
    ...
    <h2 class="text-lg font-bold text-gray-700 dark:text-yt-text transition-colors duration-300">Editar usuario</h2>
</div>
...
<div class="bg-gray-200 dark:bg-yt-surface rounded-xl border border-gray-300 dark:border-yt-border p-4 sm:p-6 w-full sm:max-w-xl transition-colors duration-300">
    <form method="POST" action="/usuarios/update/<?= $usuario['id'] ?>">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-yt-text mb-1 transition-colors duration-300">
            ...
            <div class="col-span-1 sm:col-span-2 flex items-center gap-3">
                <input type="checkbox" name="estado" id="estado" <?= $usuario['estado'] ? 'checked' : '' ?> class="w-4 h-4 accent-indigo-600">
        ...
        <?php
        $permisosActivos = !empty($usuario['permisos'])
            ? array_map('trim', explode(',', $usuario['permisos']))
            : array_map('trim', explode(',', $rol['permisos'] ?? ''));
                        ...
                        <tbody class="divide-y divide-gray-300 dark:divide-yt-border">
                            ...
                            <tr>
                                ...
                                <?php foreach ($acciones as $accion): ?>
                                    ...
                                    <?php $permiso = $key . '.' . $accion; $inputKey = $key . '_' . $accion; $checked = in_array($permiso, $permisosActivos) ? 'checked' : ''; ?>
                                    <td class="px-3 py-2.5 text-center">
                                        <input type="checkbox" name="<?= $inputKey ?>" <?= $checked ?> class="w-4 h-4 accent-indigo-600">
                                    </td>
                                <?php endforeach; ?>
                                <td class="px-3 py-2.5 text-center">
                                    <?php if ($key === 'ventas'): ?>
                                        <?php $checked = in_array('ventas.cancelar', $permisosActivos) ? 'checked' : ''; ?>
                                        <input type="checkbox" name="ventas_cancelar" <?= $checked ?> class="w-4 h-4 accent-indigo-600">
                                    <?php endif; ?>
                                </td>
                            </tr>
                            <?php endforeach; ?>
        ...
        <div class="flex items-center gap-3 mt-6 pt-6 border-t border-gray-100 dark:border-yt-border transition-colors duration-300">
            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-6 py-2.5 rounded-lg transition-colors duration-300">Actualizar usuario</button>
