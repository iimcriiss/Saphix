...
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-lg font-bold text-gray-700 dark:text-yt-text transition-colors duration-300">Proveedores</h2>
        <p class="text-sm text-gray-500 dark:text-yt-muted mt-0.5 font-medium transition-colors duration-300">Gestiona tus proveedores</p>
    </div>
    ...
    <div class="flex items-center gap-4">
        <div class="relative">
            ...
            <div id="export-menu" class="hidden absolute right-0 mt-1 w-48 bg-white dark:bg-yt-surface rounded-lg shadow-lg border border-gray-300 dark:border-yt-border z-50 overflow-hidden transition-colors duration-300">
                <a href="/exportar/proveedores/xlsx" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-neutral-900 transition-colors duration-300">
                ...
                <a href="/exportar/proveedores/csv" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-neutral-900 transition-colors duration-300">
                    <svg class="w-4 h-4 text-blue-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    CSV (.csv)
                </a>
                <a href="/exportar/proveedores/pdf" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-neutral-900 transition-colors duration-300">
        ...
        <!-- Botón nueva venta -->
        <?php if (Permission::can('proveedores.crear')): ?>
            <a href="/proveedores/create" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors duration-300">
                + Nuevo proveedor
            </a>
        <?php endif; ?>
    </div>
    ...
    <table class="w-full text-sm transition-colors duration-300">
        <thead class="bg-gray-200 dark:bg-yt-elevated transition-colors duration-300">
            <tr>
                ...
                <th class="text-left px-6 py-3 text-md text-gray-500 dark:text-gray-400 font-semibold transition-colors duration-300">Contacto</th>
        ...
        <tbody class="divide-y divide-gray-300 dark:divide-yt-border transition-colors duration-300">
            <?php if (empty($proveedores)): ?>
                <tr>
                    <td colspan="6" class="px-6 py-10 text-center text-gray-400 dark:text-gray-500 transition-colors duration-300">
            ...
            <?php else: ?>
                <?php foreach ($proveedores as $proveedor): ?>
                    <tr class="hover:bg-gray-100 dark:hover:bg-gray-700/30 transition-colors duration-300">
                        <td class="px-6 py-4">
                            <p class="font-medium text-gray-700 dark:text-gray-100 transition-colors duration-300"><?= $proveedor['nombre'] ?></p>
                            <p class="text-xs text-gray-400 mt-0.5 transition-colors duration-300"><?= $proveedor['direccion'] ?? '—' ?></p>
                        </td>
                        ...
                        <td class="px-6 py-4 flex items-center gap-3">
                            <?php if (Permission::can('proveedores.editar')): ?>
                                <a href="/proveedores/edit/<?= $proveedor['id'] ?>" class="text-indigo-500 hover:text-indigo-700 text-xs font-bold transition-colors duration-300">Editar</a>
                            <?php endif; ?>
                            <?php if (Permission::can('proveedores.eliminar')): ?>
                                <a href="/proveedores/delete/<?= $proveedor['id'] ?>" class="text-red-400 hover:text-red-600 text-xs font-bold transition-colors duration-300" onclick="return confirm('¿Eliminar este proveedor?')">Eliminar</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
    ...
    <?php else: ?>
        ...
        <div class="bg-ytlight-surface dark:bg-yt-surface rounded-xl border border-gray-300 dark:border-yt-border p-4 shadow-sm transition-colors duration-300">
            <div class="flex items-start justify-between mb-3">
                <div>
                    <p class="font-semibold text-gray-700 dark:text-yt-text"><?= $proveedor['nombre'] ?></p>
                    <p class="text-xs text-gray-500 dark:text-yt-muted"><?= $proveedor['direccion'] ?? '—' ?></p>
                </div>
                <?php if ($proveedor['estado']): ?>
            ...
            <div class="grid grid-cols-2 gap-2 text-xs text-gray-500 dark:text-yt-muted mb-3">
                <div><span class="font-medium text-gray-600 dark:text-gray-400">Contacto:</span> <?= $proveedor['contacto'] ?? '—' ?></div>
                <div><span class="font-medium text-gray-600 dark:text-gray-400">Teléfono:</span> <?= $proveedor['telefono'] ?? '—' ?></div>
                <div class="col-span-2"><span class="font-medium text-gray-600 dark:text-gray-400">Email:</span> <?= $proveedor['email'] ?? '—' ?></div>
            </div>
